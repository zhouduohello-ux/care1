...
- 以下情况可覆盖：
  ...
  - 复合信号冲突
  - 患者主动求助
- 异常模式下最多追加 3 个问题（`EXCEPTION_QUESTIONS` 长度）；
- 异常模式终止规则：
  - 澄清 3 个问题后自动结束，并发送标准安全提示；
  - 若用户报告严重呼吸困难、胸痛、意识模糊、严重过敏反应或症状急剧恶化，立即结束 check-in 并建议联系医生或拨打 999；
  - 所有异常交互记录为 `adverse_event` observation，写入 Disease Card Adverse Events 模块。
