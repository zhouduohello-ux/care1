...
**当前状态**：已决策并落地。`packages/engine/src/planner.ts` 在异常模式下最多追加 3 个澄清问题（`EXCEPTION_QUESTIONS`），之后自动结束并发送标准安全提示；感知层识别到严重呼吸困难、胸痛、意识模糊、严重过敏反应或症状急剧恶化时，由 L6 安全层立即返回紧急提示；所有异常交互以 `adverse_event` observation 写入 Disease Card。详见 `docs/decisions.md` D7。
...
| 优先级 | 项 | 状态 |
|--------|-----|------|
| P1 | B1, B2, B3, B7, B8, B10, B11, B15 | 已实现 |
