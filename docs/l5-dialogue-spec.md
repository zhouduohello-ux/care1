...
- **状态**：✅ 已实现。实现位置：`packages/db/prisma/schema.prisma`、`packages/engine/src/turn-manager.ts`、`packages/engine/src/engine.ts`；单元测试：`packages/engine/src/turn-manager.test.ts`。
