...
- **目标**：当用户明确希望跳过当前问题时，系统记录 `no_answer` 并继续下一题，而不是反复重提示。
- **方案**：在 `packages/engine/src/turn-manager.ts` 中新增 `looksLikeSkipRequest()` 识别常见跳过短语（如 "skip" / "next question" / "I don't want to answer" 等）；命中时通过 `recordSkippedQuestion()` 写入 `subjective/{topic}=no_answer` observation，清空 `pendingQuestion`，并写入 `user_action` Event 标注跳过原因；`packages/engine/src/engine.ts` 在澄清判断之后、重提示之前处理跳过意图。
