...
1. L4 Planner 生成 `ask` action 后，`pendingQuestionFromPlannerOutput()` 提取 pending question 并通过 `saveOutboundMessages()` 写入 `outbound_message` Event 的 `_pendingQuestion` 字段。
2. 用户下次回复时，`engine.ts` 在调用 L4 Planner 之前调用 `getPendingQuestion()` 读取待回答问题。
3. `isAnswerToPendingQuestion()` 根据消息类型、选项 ID、scale 数字或 perception 提取的 observation 判断用户是否回答了该问题：
   - `text` 类型问题：任何文本都视为回答；
   - `single_choice`：button/list reply 必须匹配选项，文本回复必须匹配选项 ID；
   - `scale`：文本必须是 1–5 的数字；
   - `multi_select`：所有 token 必须匹配选项；
   - `question` / `help` / `stop` / `continue_cycle` 等系统意图不视为回答。
4. 若未回答，`buildRepromptMessage()` 使用相同的 topic/options 重新渲染一条提示消息（前缀为 "I didn't catch that."），并跳过 L4 Planner，不消耗预算。

### 4.5 代码调用点

- **用户入站**：`packages/engine/src/engine.ts`（L4 Planner 之前）
- **定时触发**：`packages/engine/src/engine.ts:handleCheckInTrigger`（保存 outbound 时附带 pending question）
