...
> **文档编号**：SPEC-L5-001  
> **版本**：v1.19
> **分支**：`feat/turn-manager-multi-turn-clarification`  
> **对应架构层**：L5 Dialogue  
...
3. `isAnswerToPendingQuestion()` 判断用户是否回答了该问题。匹配逻辑分层：
   ...
   - `multi_select`：按逗号 / "and" / "or" / 空格分词并过滤 stop words，先匹配选项 ID，再匹配 `dialogue-locales` 中的 `optionSynonyms`；若命中部分选项但仍有无法识别的 meaningful 词语，视为 **partial answer**，先保存已命中选项并发送澄清追问，而不是直接重提示；
