...
### L5-FOLLOW-021 延迟问题运营指标（待实现）
- **目标**：在 `/admin/metrics` 暴露当前 cycle/check-in 的延迟问题数量与话题转移次数，便于运营分析用户对话偏离模式。
- **方案**：
  - `turnManager` 块新增 `deferredQuestions24h`（最近 24h 被延迟的问题数）与 `deferredQuestionsReRaised24h`（被重新提出的问题数）。
  - 通过统计 `user_action` Event 中 `action="topic_shift"` 与 `action="deferred_question_reraised"` 实现。
- **状态**：⏳ 待实现。
