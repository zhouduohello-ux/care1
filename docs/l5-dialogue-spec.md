...
> **文档编号**：SPEC-L5-001  
> **版本**：v1.16  
> **分支**：`feat/l5-spec-sync-turnmanager`  
> **对应架构层**：L5 Dialogue  
...
`apps/api/src/services/scheduler.ts` 提供 `getNudgeAfterMs()` 与 `getPendingTimeoutMs()` 读取环境变量；`processPendingNudges` / `processExpiredPendingQuestions` 支持通过参数覆盖，便于测试与本地测试工具。
...
### L5-FOLLOW-004 Turn Manager 重提示对 Planner 不可见
- **目标**：修复 `CheckIn.repromptCount` 更新后 L4 Planner 无法读取，导致 `no_answer` observation 可能重复生成的问题。
- **方案**：在 engine 主流程中把 `repromptCount` 与 `pendingQuestion` 一起传递给 planner；在 `MAX_REPROMPTS` 超限时由 Turn Manager 提前记录 `no_answer` 并清空 pending，Planner 接收到已清空状态后继续下一题。
- **状态**：✅ 已实现。实现位置：`packages/engine/src/engine.ts`、`packages/engine/src/turn-manager.ts`。
...
| 2026-07-11 | v1.15 | L5 轮次管理文档补齐：新增 pending question 24h 静默超时流程说明，记录 scan-expired-pending scheduler job、超时 observation/state_updated Event 语义；补充 L5-FOLLOW-004/005 实现状态 | AI Agent |
